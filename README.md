...
`npm run config-push:dry-run` audits the Master `CONFIG` snapshot against every active employee source. Source seeding reads Master `2. NHAN SU` through the read-only Google API: `TÌNH TRẠNG` is authoritative, `Đang làm` and blank are active, and `Đã nghỉ`/`inactive`/`disabled`/`retired`/`archived`/`offboarded` are inactive. Employee files absent from Master are logged and skipped. Discovery performs no Google writes; config-push dry-run performs no Google writes.
...
`npm run sync:readonly` creates an atomic, locked Master snapshot under `reports/phase4` from the repository's extracted CSV fixtures. It never invokes an importer, SQL, or a Google write API. Set `MASTER_FIXTURE_DIR` to use another fixture directory. Live export is explicitly opt-in with `READONLY_LIVE=1` plus a readable `GOOGLE_APPLICATION_CREDENTIALS` file or the JSON secret env `GOOGLE_APPLICATION_CREDENTIALS_JSON` (`GOOGLE_SERVICE_ACCOUNT_JSON` alias); the exporter requests only Sheets read-only and Drive metadata read-only scopes, reads each sheet in bounded row ranges (500 rows by default), and retries HTTP 429 responses with backoff. It spools each completed sheet atomically and does not lock or fingerprint the final snapshot until every sheet has been processed. Set `READONLY_COMPARISON_SNAPSHOT` to emit JSON and Markdown discrepancy audits by post URL, post URL/brand, and staff ID/status.
...
Manual production controls, the signed Apps Script bridge, Render secrets, and the one-time queue migration are documented in [docs/admin-sync-control.md](docs/admin-sync-control.md). Deploying the control plane does not enqueue a job, and the Render blueprint keeps CONFIG production writes disabled by default.
