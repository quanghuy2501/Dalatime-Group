...
- `npm run seed:nv-sources` reads Master `2. NHAN SU` through the read-only Sheets API and persists its verbatim `TÌNH TRẠNG` plus registry presence. `Đang làm` and blank are active; `Đã nghỉ`, `inactive`, `disabled`, `retired`, `archived`, and `offboarded` are inactive. Employee files absent from Master are logged and skipped. No employee IDs or source-count range are hardcoded. The Master file ID is rejected.
...
Run `npm run migrate:nv`, then seed `nv_ingestion_sources`. Migrations 007 and 010 bootstrap config provenance and add Master-registry presence tracking. Each later successful Master sync atomically becomes the active `config_version` with the exact 22-pair mapping; ingestion refuses arbitrary or failed-run config.
