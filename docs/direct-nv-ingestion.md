...
Required secrets are `DATABASE_URL` and `GOOGLE_APPLICATION_CREDENTIALS`; retain the existing `MASTER_SPREADSHEET_ID` exclusion guard. Runtime controls are `NV_CONCURRENCY` (maximum 3), `NV_PAGE_ROWS`, `NV_PAGE_TIMEOUT_MS` and `NV_SOURCE_TIMEOUT_MS` (both default 120000), `NV_TOTAL_TIMEOUT_MS` (default 1500000), `NV_HEARTBEAT_MS` (default 30000), `NV_SOURCE_RETRIES`, `GOOGLE_REQUESTS_PER_MINUTE` (default 55, capped at 60), `GOOGLE_READ_DELAY_MS`, and `GOOGLE_MAX_RETRIES`. The command emits one-line JSON progress for every source/page, periodic heartbeats, and exactly one terminal `published` or `blocked` event. Do not add Sheets write scopes or token-rotation logic.
