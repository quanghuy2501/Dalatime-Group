...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  try { const result = await seedNvSources({api, db}); console.log(JSON.stringify({status:'ok', discovered:result.sources.length, active:result.sources.map(x=>x.nv_id), skipped:result.skipped}, null, 2)); }
